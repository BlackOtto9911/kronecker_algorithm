# kronecker_algorithm_ver_1_0.py
# ver 1.0.0
...
def get_all_combinations(points, pow): # pow - максимальная степень искомого полинома разложения
    result = {}
...
def print_polynomial_ver_1_0(p):
    if not p:
        return "0"
...
def kronecker_factorization_ver_1_0(polynomial):
    global count_lagrange
    count_lagrange = 0
    ...
    # a = 0,1,2,3 ... без сортировки делителей
    while is_polynomial(polynomial):
        # m + 1
        n = len(polynomial) - 1
        m = n // 2 + 1

        # находим точки, массив a
        a = [a_i for a_i in range(m)]

        # значения полинома для каждого a
        values = [get_polynomial_value(polynomial, a_i) for a_i in a]
